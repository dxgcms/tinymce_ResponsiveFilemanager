...
/**
	DXGCMS ADDED
	$document_root to ensure that theres a variable for it with an ending slash
**/
...
/**
	DXGCMS ADDED
	$max is to count how many ../ to add to get to the root folder
**/
$max = count(explode('/', __DIR__))-count(explode('/', $document_root))-1;
$startpath = '';
/**
	DXGCMS ADDED
	$startpath is to being executed to ensure that there's enough ../ to get to the root folder.
**/
for($i=1; $i <= $max; $i++) {
	$startpath .= '../';
}
...
/**
	DXGCMS ADDED
	$folder to ensure that there's a slash at the begining and the end.
**/
if(substr($folder, -1) != '/') {
	$folder .= '/';
}
...
$upload_dir = $startpath.$folder; // path from base_url to base of upload folder (with start and final /)
$current_path = $startpath.$folder; // relative path from filemanager folder to upload folder (with final /)
//thumbs folder can't put inside upload folder
$thumbs_base_path = $startpath.$folder.'thumbs/'; // relative path from filemanager folder to thumbs folder (with final /)
...
/**
	DXGCMS ADDED
	All FALSE except the UPLOAD to ensure that it's locked down for people who shouldn't be doing stuff.
**/
$delete_files	 	  = FALSE;
